...
//    public ArrayUnsortedList<T> slice(int F , int L){
//          ArrayUnsortedList<T> temp= new ArrayUnsortedList();
//          if(F<=0 || L<=0 || F<= numElements || L<=numElements )
//              return null;
////          F-=1 ;
////          L-=1;
//         // else if((F > 0 && L > 0 ) && (F <= numElements) ){
//          else{
////          int counter=0;
////          while(list != null && counter <=F){
////             counter++; 
////          }
...
////             
//       
       /*  Write an instance method named slice that will be incorporated 
           into the ArrayUnsortedList class. This method should accept two
           parameters, F and L, which should be of type integer. The method
           should return the elements, in a list of type ArrayUnsortedList,
           between F and L inclusive (i.e. First and Last elements are included).
           Make sure that F and L are greater than 0 and less than or equal to 
           the number of elements, otherwise return null.
       ...
       */
        public ArrayUnsortedList<T> slice(int F , int L){
            ...
            if(F<=0 || L<=0 || F>numElements || L>numElements || F>L ) {
                return null;
            }
